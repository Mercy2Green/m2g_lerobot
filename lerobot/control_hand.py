...
def hand_start():
    ...
    write6(ser, 1, 'speedSet', [500, 500, 500, 500, 500, 500]) # ID号改为对应灵巧手的ID，val对应的电缸ID为1,2,3,4,5,6;对应的速度值为0-1000，1000为最大值，0不运动，如果val设置为-1，相应的手指无反应
    time.sleep(1)                   # 延时1s
    print('设置灵巧手抓握力度参数！')
...
def hand_control(ser):
    #ser = openSerial('/dev/ttyUSB0', 115200)
    print('设置灵巧手运动角度参数0，-1为不设置该运动角度！')
    
    hand_read(ser)  # 读取当前角度值
    time.sleep(2) 
    # write6(ser, 1, 'angleSet', [1000, 1000, 1000, 1000, 1000, 1000])
    # time.sleep(2) 
    # write6(ser, 1, 'angleSet', [1000, 1000, 1000, 1000, 500, 1000])
    # time.sleep(2) 
    write6(ser, 1, 'angleSet', [1000, 0, 1000, 1000, 0, 1000])
    time.sleep(2)                   # 延时1s
    print('运行灵巧手当前序列动作！')
def hand_read(ser):
    #ser = openSerial('/dev/ttyUSB0', 115200)
...
if __name__ == '__main__':
    ...
    print("关闭串口")
    ser.close()
    # ser = hand_start()  # 打开串口并设置灵巧手参数
    # time.sleep(1)  # 等待串口准备就绪
    # hand_control(ser)  # 控制手部动作
    # time.sleep(1)  # 等待手部动作完成
    # hand_read(ser)  # 读取手部状态
    # print("手部控制完成，关闭串口")
