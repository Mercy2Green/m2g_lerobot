'''
#hyx
1. Transform action : joint to end-effector pose + gripper value.
2. Convert LeRobot dataset to RLDS format.
3. only transform the dataset to train split, no validation or test splits.(待修改成支持valid)
'''
...
def generate_config_from_features(features, encoding_format, **kwargs):
    ...
    action_info = tfds.features.Tensor(
        shape=(7,), 
        dtype=np.float32, 
        doc="eef_pose(xyz+quaternion) + gripper"
    )
...
def parse_step(data_item):
    ...
    # action_info = action_info if len(action_info) > 1 else action_info.popitem()[1]
     ...
     # === Action: joint -> eef + gripper ===
    # 取 joint（前6维）和 gripper（第7维）
    
    action_tensor = data_item["action"]  # tensor of shape (7,)
    '''
    joint_angles = (action_tensor[:6] * 0.001).tolist()
    gripper_value = action_tensor[6].item()

    # Forward Kinematics
    j_pos = fk_solver.CalFK(joint_angles)
    ee_pose = (np.array(j_pos[5]) * 1000).tolist()  # eef position and orientation

    # 拼接 eef + gripper，作为新的 action
    
    action_info = ee_pose + [gripper_value]
    '''
    action_info = action_tensor.tolist()
...
'''python lerobot2rlds.py \
    --src-dir /home/hpx/vla/src/dataset/cube3_tcp \
    --output-dir /home/hpx/vla/src/dataset/rlds/cube3_tcp \
    --task-name "Grab the pink cube and put it in the box"  \
'''